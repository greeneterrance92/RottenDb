package com.reimbursement_app_1.jsp;
 ...
 */
@WebServlet("/ReimbursementControllerServlet2")
public class ReimbursementControllerServlet2 extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	private ReimburseDbUtil reimburseDbUtil;
	private ReimburseDbUtil2 reimburseDbUtil2;
	 ...
	 super.init();
	  ...
	  // create our reimburse db util and pass in the connection pool
	 	...
	 	try {
			 reimburseDbUtil2 = new ReimburseDbUtil2();
		 	}
		 	catch (Exception exc) {
		 		throw new ServletException(exc);
		 	}
	...
	private void loadReimbursement(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/update-reimbursement-form2.jsp");
	...
	private void listReimbursements(HttpServletRequest request, HttpServletResponse response)
		...
		List<Reimbursement> reimbursements = reimburseDbUtil2.getReimbursements(request, response); 
				// add employees to the request
				request.setAttribute("REIMBURSEMENTS_LIST", reimbursements);
				// send to JSP page (view)
				RequestDispatcher dispatcher = request.getRequestDispatcher("/list-reimbursements2.jsp");
				dispatcher.forward(request, response);
				return;
	}
